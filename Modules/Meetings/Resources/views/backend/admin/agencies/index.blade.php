        ...
        <div class="card-body">
            <table class="table aiz-table mb-0">
                ...
                <tr>
                    ...
                    <th>{{translate('Campany Image')}}</th>
                    <th data-breakpoints="lg">{{translate('Agency Name ')}}</th>
                    <th data-breakpoints="lg">{{translate('Seller Name')}}</th>
                    <th data-breakpoints="lg">{{translate('Seller Email')}}</th>
                    <th data-breakpoints="lg">{{translate('Approval')}}</th>
                    <th data-breakpoints="lg">{{ translate('Num. of Countries') }}</th>
                    <th data-breakpoints="lg">{{ translate('Num. of Joins') }}</th>
                    ...
                    <tr>
                        <td>
                            <div class="form-group">
                                <div class="aiz-checkbox-inline">
                                    <label class="aiz-checkbox">
                                        <input type="checkbox" class="check-one" name="id[]" value="{{$agency->id}}">
                                        <span class="aiz-square-check"></span>
                                    </label>
                                </div>
                            </div>
                        ...
                        <td>
                            <img class="img-thumbnail rounded img-company" style="width:40px" src="{{ uploaded_asset($agency->campany_image) }}">
                        </td>
                        <td>
                            {{ $agency->name }}
                        </td>
                        ...
                        <td>
                            {{ $agency->agency_country_count }}
                        </td>
                        <td>
                            {{ $agency->agency_joins_count }}
                        </td>
                        <td>
                            <a class="btn btn-soft-success btn-icon btn-circle btn-sm" href="{{route('admin.agencies-show', ['id'=>$agency->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                    @can('view_seller_profile')
                                        <a href="#" onclick="show_seller_profile('{{$agency->seller->shop->id}}');"  class="dropdown-item">
                                            {{translate('Profile')}}
                                        </a>
                                    @endcan
                                    @can('login_as_seller')
                                        <a href="{{route('sellers.login', encrypt($agency->seller->shop->id))}}" class="dropdown-item">
                                            {{translate('Log in as this Seller')}}
                                        </a>
                                    @endcan
                                    @can('pay_to_seller')
                                        <a href="#" onclick="show_seller_payment_modal('{{$agency->seller->shop->id}}');" class="dropdown-item">
                                            {{translate('Go to Payment')}}
                                        </a>
                                    @endcan
                                    @can('seller_payment_history')
                                        <a href="{{route('sellers.payment_history', encrypt($agency->seller->id))}}" class="dropdown-item">
                                            {{translate('Payment History')}}
                                        </a>
                                    @endcan
                                    
                                    @can('ban_seller')
                                        @if($agency->seller->banned != 1)
                                            <a href="#" onclick="confirm_ban('{{route('sellers.ban', $agency->seller->shop->id)}}');" class="dropdown-item">
                                            {{translate('Ban this seller')}}
                                            <i class="fa fa-ban text-danger" aria-hidden="true"></i>
                                            </a>
                                        @else
                                            <a href="#" onclick="confirm_unban('{{route('sellers.ban', $agency->seller->shop->id)}}');" class="dropdown-item">
